...
from tensorflow.keras.layers import Conv2DTranspose, Conv2D, Conv1D, BatchNormalization, \
    Activation, Input, UpSampling2D, MaxPooling2D, MaxPooling1D, SpatialDropout2D, Lambda, Wrapper
from tensorflow.keras import layers
from SpNormalization import SpectralNormalization
...
def tomogan_disc(input_shape):
    ...
    _tmp = Conv2D(filters=64, kernel_size=3, padding='same', strides=(2, 2), \
                  activation=tf.keras.layers.LeakyReLU(alpha=0.01))(_tmp)
    ...
    # _tmp = BatchNormalization(axis=-1, momentum=0.99, epsilon=0.001, center=True, scale=True, beta_initializer='zeros', gamma_initializer='ones', moving_mean_initializer='zeros', moving_variance_initializer='ones')(_tmp)
...
def unet(input_shape, use_cnnt=False, nlayers=3):
    ...
    _tmp = unet_conv_block(ly_outs[-1], 32)#32
    ly_outs.append(_tmp)
    ...
    for ly, nch in zip(range(1, nlayers + 1), (64, 32, 32, 16, 16, 8)):  # (64, 32, 32)
        if use_cnnt:
